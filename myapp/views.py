...
'''
def adjust_price_view(request):
    """
    多商品一次推論。
    - 前端以 AJAX (或表單) 傳入 product_list: [{product_idx, current_buy_price, ...}, {...}, ...]
    - 後端呼叫 inference_multiple_products
    - 回傳 JSON 結果
    """
    if request.method == "POST":
        try:
            # 假設前端用 AJAX 傳 JSON，若改用普通 form-data 也可，但需自行解析
            # 這裡示範從 request.POST 或 request.body 中取 JSON
            import json
            raw_body = request.body.decode('utf-8')
            data = json.loads(raw_body)  # 取得前端 JSON
            products_info = data.get("product_list", [])

            # 呼叫多商品一次推論
            results = dqn_store.inference_multiple_products(products_info)
        ...
        # GET 請求，顯示多商品一次推論的頁面
        return render(request, "adjust_price_form.html")
...
import requests
# 這是新的 import
#from myapp.cgan_module.generate_cgan_data import generate_data_for_buy, generate_data_for_sell
#from myapp.cgan_module.cgan_buy_training import train_cgan_for_buying
#from myapp.cgan_module.cgan_sell_training import train_cgan_for_selling
...
def start_training_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            # 1) 產生 "買" 的數據 => "my_data_buy.csv"
            generate_data_for_buy(csv_filename_buy="my_data_buy.csv", num_records=500)
            # 2) 訓練 cGAN (買) => 產生 "cgan_generator_buy.pth"
            train_cgan_for_buying(csv_filename_buy="my_data_buy.csv")

            # 3) 產生 "賣" 的數據 => "my_data_sell.csv"
            generate_data_for_sell(csv_filename_sell="my_data_sell.csv", num_records=500)
            # 4) 訓練 cGAN (賣) => 產生 "cgan_generator.pth"
            train_cgan_for_selling(csv_filename_sell="my_data_sell.csv")
            
            # 5) 最後才開始背景 DQN 訓練
            start_background_training(num_episodes, max_steps, product_count)

            message = "完成：已生成 BUY / SELL cGAN 資料 & 訓練完，最後已開始 DQN 背景訓練。"
...
def server_status_view(request):
    market_api_url = "https://cba3-59-125-237-248.ngrok-free.app/api/market/status"
    player_api_url = "https://cba3-59-125-237-248.ngrok-free.app/api/player/status"

    market_data = []
    player_data = {}
    error_message = None

    # Ngrok warning bypass header (for browser fetch calls)
    headers = {
        "ngrok-skip-browser-warning": "true"
    }
    ...
    try:
        ...
        market_resp = requests.get(market_api_url, headers=headers, timeout=5)
        market_resp.raise_for_status()
        market_data = market_resp.json()  # → [{ "player": ..., "prices": [...], "products": {...}, "type": ... }, …] :contentReference[oaicite:1]{index=1}

        # 2) Fetch player status
        player_resp = requests.get(player_api_url, headers=headers, timeout=5)
        player_resp.raise_for_status()
        player_data = player_resp.json()  # → { "online": 3, "todayOnline": 18 } :contentReference[oaicite:2]{index=2}

    except requests.Timeout:
        error_message = "請求超時，伺服器可能忙碌中"
    except ValueError as e:
        # .json() 解析失敗會丟 ValueError
        error_message = f"JSON 解析錯誤：{e}"
    except requests.RequestException as e:
        # 包括 ConnectionError, HTTPError, etc.
        error_message = f"無法取得資料：{e}"
...
'''
先更新 dqn_store.buy_price_bounds & dqn_store.sell_price_bounds：
這樣 generate_data_for_buy/for_sell 就能拿到最新上下限來生成 CSV。

generate_data_for_buy(...)：

預設寫入 my_data_buy.csv。

之後 train_cgan_for_buying(csv_filename_buy="my_data_buy.csv") 會去讀該檔案並訓練「買 cGAN」。

訓練完成後產生 cgan_generator_buy.pth。

generate_data_for_sell(...)：

寫入 my_data_sell.csv。

train_cgan_for_selling(csv_filename="my_data_sell.csv") 會去讀該檔案，產生 cgan_generator.pth。

最後 start_background_training(...)：

這裡才呼叫你原本的 _background_training 來執行 DQN；它會在背景 thread 跑，前端可用 /training_status_api 查詢進度。

如果 cGAN 部分也想改成背景執行，你需要自己在 tasks.py 新增 _background_cgan_buy_training / _background_cgan_sell_training 之類再說；目前是同步執行 cGAN，做完才進入 DQN。
